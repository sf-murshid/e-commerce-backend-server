...
route.post('/register', async (req, res) => {
    ...
    if (!hashPassword) return res.status(400).send({ message: "hashing password failed !!" })
    ...
    if (!user) {
        return res.status(400).json({ success: false, message: "new user creation failed !!" })
    }
...
route.put('/:id', async (req, res) => {
    ...
    if (req.body.password) {
        ...
        if (!newPassword) return res.status(400).send({ message: "hashing password failed !!" })
    }
    else {
    ...
    if (!user) {
        return res.status(400).json({ success: false, message: "user updatation failed !!" })
    }
